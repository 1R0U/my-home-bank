...
type MapStore = {
  ...
  /** 町の固定物 ＋ 家族の家 ＋ 置いた装飾。描画に使う */
  objects: MapObject[];
  /** DBから読み込んだ、置いた装飾 */
  placedDecorations: MapObject[];
  /** 家族の家を差し替える。`objects` も合わせて作り直す */
  setFamilyHouses: (houses: MapObject[]) => void;
  /** 置いた装飾を差し替える。`objects` も合わせて作り直す */
  setPlacedDecorations: (decorations: MapObject[]) => void;
};

/**
 * 描画に渡す一覧を組み立てる。**並び順は固定物→家→装飾。**
...
const buildObjects = (
  familyHouses: MapObject[],
  placedDecorations: MapObject[],
): MapObject[] => [...INITIAL_MAP_OBJECTS, ...familyHouses, ...placedDecorations];
...
export const useMapStore = create<MapStore>((set) => ({
  ...
  placedDecorations: [],
  setFamilyHouses: (houses) =>
    set((state) => {
      if (sameObjects(state.familyHouses, houses)) return {};
      return {
        familyHouses: houses,
        objects: buildObjects(houses, state.placedDecorations),
      };
    }),
  setPlacedDecorations: (decorations) =>
    set((state) => {
      ...
      return {
        objects: buildObjects(state.familyHouses, decorations),
