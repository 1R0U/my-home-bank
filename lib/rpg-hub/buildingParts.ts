 ...
 *
 * **色の差し替え枠（Issue #254）。** ログイン中の本人の見た目を `palette` で当てる。
 *   - `skin` … 体の明るい緑（胴・頭・まぶた）
 *   - `accent` … 濃い緑（足・背中の斑点・口・鼻の穴）
 *   - `hair` … カエルには無いので使わない
 * のど・白目・瞳は固定にして、どの色でも顔が見分けられるようにしてある。
 * 枠を付けても既定色は変えていないので、見た目の指定が無い人は今までどおりのカエルになる。
 */
export const PLAYER_PARTS: BuildingPart[] = [
  // 後ろ足（もも → 足先）。体の横で畳んで、足先を前へ出す
  ...[-0.36, 0.36].map((x) => withSlot(box(0.2, 0.26, 0.34, { x, y: -0.12, z: -0.26 }, "#2f7a2a"), "accent")),
  ...[-0.36, 0.36].map((x) => withSlot(box(0.26, 0.1, 0.32, { x, y: -0.28, z: 0.02 }, "#2f7a2a"), "accent")),
  // 前足（腕 → 手）。頭の下から前へ突く
  ...[-0.3, 0.3].map((x) => withSlot(box(0.13, 0.2, 0.13, { x, y: -0.18, z: 0.46 }, "#2f7a2a"), "accent")),
  ...[-0.32, 0.32].map((x) => withSlot(box(0.18, 0.09, 0.22, { x, y: -0.29, z: 0.56 }, "#2f7a2a"), "accent")),
  // 胴（背中）。頭より低く細くして、上から見たときに頭との段差を作る
  withSlot(box(0.66, 0.34, 0.5, { x: 0, y: -0.08, z: -0.28 }, "#4fae3f"), "skin"),
  // 背中の斑点。等間隔に並べないことで、のっぺりした箱に見えないようにする
  withSlot(box(0.16, 0.06, 0.14, { x: -0.15, y: 0.09, z: -0.22 }, "#2f7a2a"), "accent"),
  withSlot(box(0.12, 0.06, 0.12, { x: 0.17, y: 0.09, z: -0.34 }, "#2f7a2a"), "accent"),
  withSlot(box(0.1, 0.06, 0.1, { x: -0.02, y: 0.09, z: -0.44 }, "#2f7a2a"), "accent"),
  // 頭。胴より大きく、高く、前に出す
  withSlot(box(0.8, 0.6, 0.56, { x: 0, y: 0.08, z: 0.22 }, "#4fae3f"), "skin"),
  // のど。頭より奥行きだけ大きくして、+Z面にクリーム色が出るようにする
  box(0.62, 0.14, 0.58, { x: 0, y: -0.14, z: 0.22 }, "#f7e9c4"),
  // 口。頭より一回り大きい帯にして、+X面と+Z面へぐるりと回り込ませる
  withSlot(box(0.84, 0.09, 0.6, { x: 0, y: -0.02, z: 0.22 }, "#2f7a2a"), "accent"),
  // 鼻の穴。真上から見たときの「顔の向き」の手がかりになる
  ...[-0.08, 0.08].map((x) => withSlot(box(0.05, 0.04, 0.05, { x, y: 0.38, z: 0.42 }, "#2f7a2a"), "accent")),
  // 目。まぶたのふくらみ → 眼球 → 瞳の3段重ね。円柱だと輪切りの角が出るので球で作る。
  // 瞳は少し前と上へ寄せて、見下ろすカメラからも前を見ているように見せる
  ...[-0.25, 0.25].map((x) => withSlot(sphere(0.3, 0.26, 0.3, 14, { x, y: 0.36, z: 0.18 }, "#4fae3f"), "skin")),
...
/**
 * プレイヤー（うさぎ）。カエル（PLAYER_PARTS）に続く2体目の選べる姿（Issue #235）。
 *
 * カエルと同じく当たり判定の直径（0.9）に横幅を収め、正面は +Z。
 * 長い耳が特徴なので、カメラに映る上面へ大きく出るようにしている。
 */
export const RABBIT_PARTS: BuildingPart[] = [
  // 後ろ足
  ...[-0.24, 0.24].map((x) => box(0.18, 0.22, 0.3, { x, y: -0.24, z: -0.14 }, "#f3ede3")),
  // 前足
  ...[-0.16, 0.16].map((x) => box(0.13, 0.24, 0.16, { x, y: -0.2, z: 0.3 }, "#f3ede3")),
  // しっぽ（丸い綿毛）
  sphere(0.22, 0.2, 0.2, 12, { x: 0, y: -0.02, z: -0.42 }, "#ffffff"),
  // 胴
  box(0.56, 0.4, 0.56, { x: 0, y: 0, z: -0.06 }, "#fdf7ee"),
  // 頭
  box(0.42, 0.4, 0.4, { x: 0, y: 0.18, z: 0.28 }, "#fdf7ee"),
  // 耳。長い箱を左右それぞれ外側へ少し倒す
  ...[-0.1, 0.1].map((x) =>
    box(0.13, 0.5, 0.08, { x, y: 0.62, z: 0.2 }, "#fdf7ee", { x: x < 0 ? -0.15 : 0.15, y: 0, z: 0 }),
  ),
  // 耳の内側
  ...[-0.1, 0.1].map((x) =>
    box(0.08, 0.4, 0.02, { x, y: 0.6, z: 0.24 }, "#f5b8c8", { x: x < 0 ? -0.15 : 0.15, y: 0, z: 0 }),
  ),
  // ほお
  ...[-0.16, 0.16].map((x) => sphere(0.13, 0.11, 0.1, 10, { x, y: 0.1, z: 0.42 }, "#fdf7ee")),
  // 鼻
  box(0.05, 0.04, 0.04, { x: 0, y: 0.2, z: 0.48 }, "#e07ba0"),
  // 目
  ...[-0.13, 0.13].map((x) => sphere(0.1, 0.12, 0.1, 12, { x, y: 0.26, z: 0.44 }, "#3a2a1e")),
];
