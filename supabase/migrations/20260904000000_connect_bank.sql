...
begin
  ...
  if p_amount <> trunc(p_amount) then
    raise exception '借入額は整数で指定してください';
  end if;
...
--      revoke execute on function bank_deposit(uuid, numeric) from public, anon;
--      revoke execute on function bank_withdraw(uuid, numeric) from public, anon;
--      revoke execute on function bank_borrow(uuid, numeric) from public, anon;
--      revoke execute on function bank_repay(uuid, numeric) from public, anon;
--   2. 各関数の先頭で auth.uid() = p_user_id を検証するガードを追加
