...
| 置いた装飾 | 家族の誰かが置いたもの（庭・家の中を問わない） | `placed_decorations` / `MapObject` | DBが持つのは「誰が・どれを・どこに・どの向きで・どの大きさで・どの家の中か」。**見た目と当たり判定の大きさはカタログから引く**。高さ（`position.y`）は保存せず、置くたびに計算する（[Issue #223](https://github.com/1R0U/my-home-bank/issues/223)）。**家族ぶんを読み込んで表示する**ので、家族が置いたものは全員に見える（[Issue #244](https://github.com/1R0U/my-home-bank/issues/244)） |
| 部屋の持ち主 | その装飾がどの家の中のものか | `placed_decorations.room_owner_id` | null なら町・庭で、座標はワールド座標。null でなければ**その人の家の中**で、座標は**部屋の中心からの相対値**。部屋の並べ方を変えても行がそのまま使えるようにしている（[Issue #244](https://github.com/1R0U/my-home-bank/issues/244)） |
...
| 家族の家 | 家族ひとりにつき1軒、住宅街に建つ家 | `createFamilyHouses` / route `"house"`（`lib/rpg-hub/mapObjects.ts`） | **町の固定物ではない。** 誰が家族か（`users.family_id`）で軒数も表札も変わるので、取得してからマップに足す（`useFamilyHouses`）。形は1種類で、**屋根の色（`palette.accent`）だけが家ごとに違う** |
| 区画 | 家を建てられる場所。住宅街の道沿いに並んでいる | `HOUSE_LOT_XS` / `MAX_FAMILY_HOUSES` | 数は決め打ちで、**家族が区画より多いと、あふれたぶんは家が建たない**。空いている区画にも自然物は生えない（人数で町の外の木の位置が変わらないようにするため） |
| 家の中（部屋） | 着せ替え（姿見）と、その家の装飾ができる場所。**家1軒につき1部屋** | `createHouseInteriors` / `HOUSE_ROOM_CENTERS`（`lib/rpg-hub/mapObjects.ts`） | 他の建物と違い、**画面遷移ではなくプレイヤーをテレポートさせて出入りする**（`ChildHomeScreen.tsx` の `enterHouse`）。座標としては町から離れた場所にあるだけの、地続きの3D空間で、壁で仕切られた「別マップ」ではない（[Issue #235](https://github.com/1R0U/my-home-bank/issues/235)）。備え付けの家具（姿見・ハンガーラック）はどの部屋も同じで、違うのは置いた装飾だけ |
| 内装 | その家の中に置かれた装飾 | `room_owner_id` が一致する `placed_decorations` | **その家の持ち主のもの。** 誰がどの端末から入っても同じに見える。置ける・しまえるのは持ち主だけ（[Issue #244](https://github.com/1R0U/my-home-bank/issues/244)） |

### 家の内装は、その家の持ち主のもの（決めたこと）

**家1軒につき部屋が1つあり、中の装飾はその家の持ち主に紐づく**（`room_owner_id`）。
父の家に入れば父の内装が、母の家に入れば母の内装が出る。読み込むのは**家族ぶん**なので、
誰がどの端末から入っても同じ内装になる。

- **置ける・しまえるのは自分のものだけ。** ほかの家族の家の中には置けず（「ほかの人の
  いえには おけません」）、他人が置いたものはしまえない。「誰の家の内装か」が崩れないようにするため
- **座標は部屋の中心からの相対値で保存する。** 部屋の位置はアプリ側の定数
  （`HOUSE_ROOM_CENTERS`）なので、並べ方を変えてもDBの行はそのまま使える
- **部屋はどの家も同じ間取り**（壁・姿見・ハンガーラック）。変わるのは置いた装飾だけ
- 家族の並び順（`users` の作成順）で部屋が決まる。並びが変わると部屋の場所も変わるので、
  取得は必ず同じ順に並べる（`fetchFamilyMembers`）

町・庭（部屋の外）に置いたものは `room_owner_id` が null で、これまでどおりワールド座標。
こちらも家族ぶんを読み込むので、**家族が庭に置いたものも全員に見える**。
