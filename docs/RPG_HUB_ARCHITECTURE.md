...
初期マップは **61オブジェクト / 155メッシュ**。内訳は建物4棟、道のタイル19枚、木12本、低木10本、岩6個、街灯6本、花壇4つ。
...
**実機確認（#195）**: Expo Go（Android実機）で表示・移動・建物への接近を確認し、**体感で重さは出ていない**。ただしこれは目視での確認で、**FPS・フレーム時間・メモリの数値は取っていない**。この構成を基準として数値を記録するのは未実施で、装飾物をさらに増やす前に一度測っておきたい（下の「技術検証時に実機で FPS、メモリ使用量、初回起動時間、入力遅延を確認する」がそれにあたる）。
