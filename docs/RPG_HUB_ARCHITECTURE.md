...
**マップへの置き方**… 次の2つはテストで固定している（目視では気づきにくく、#214 でストアに低木がめり込んでいるのを見落としていた）。
...
初期マップは **158オブジェクト / 690メッシュ**（地面とプレイヤーを含む）。内訳は建物4棟、道のタイル44枚、木28本、低木22本、草むら32個、岩16個、街灯6本、花壇4つ、NPC2体、プレイヤー1体。

**ただし描画のもとになる実メッシュは144個だけで、残りの546個はインスタンス。** 木も低木も道のタイルも、同じ `model` なら形も色も完全に同じなので、1体目のメッシュを共有元にして2体目以降は `createInstance` で作っている（`decorationSources`）。Babylon はインスタンスをまとめて1回で描くため、**装飾物を増やしてもドローコールが増えない**。#214 で装飾物を倍以上に増やしたが、実メッシュは361個から144個へ減った。

共有するのは装飾物だけ。建物は1棟ずつ形が違い、NPCは `palette` で色が変わるため共有できない。

影を描くパスで深度だけをもう一度描くのは137メッシュ（インスタンスは共有元と一緒に描かれるので、共有元だけ登録する）。
