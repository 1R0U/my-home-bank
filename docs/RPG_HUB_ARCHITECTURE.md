...
照明は**上を向いた面の明るさが合計でほぼ 1.0 になる**強さにしている。1.0 を超えると素材の色がそのまま出ず、明るい色から順に白へ潰れる（#214 以前は実質2.15倍で、地面と道が真っ白だった）。平行光は X方向とZ方向で当たり方を変え、カメラから見える2面が同じ明るさにならないようにしている。
...
初期マップは **63オブジェクト / 181メッシュ**。内訳は建物4棟、道のタイル19枚、木12本、低木10本、岩6個、街灯6本、花壇4つ、NPC2体。
