...
- プレイヤーキャラクターは、建物・住人と同じパーツ組み立て（`buildingParts.ts`）で作る（6.5節）
  - 当初はリギング済みの3Dモデル（`.glb` 等）を使う予定だったが、#256 で検証して「今は変えない」と決めた。理由と、見直すきっかけは [`docs/RPG_HUB_CHARACTER_GLB_VALIDATION.md`](RPG_HUB_CHARACTER_GLB_VALIDATION.md)
- 建物を各機能への入口として扱う
  - 建物のタップで画面遷移できる
...
`.glb`（リギング済み3Dモデル）への差し替えは #256 で検証し、**今は変えない**と決めた。この固定カメラでは細かいモデルの利点が出ず、色替えを成り立たせるにはモデルを自分たちで作る必要があるため。詳細と、差し替えるときに必要なことは [`docs/RPG_HUB_CHARACTER_GLB_VALIDATION.md`](RPG_HUB_CHARACTER_GLB_VALIDATION.md)。
...
**色はログイン中の本人のものにする（#254）。** カエルのパーツにも住人と同じ色の差し替え枠（`paletteSlot`）を付けてある。体の明るい緑が `skin`、足・斑点・口などの濃い緑が `accent` で、`hair` は使わない。のど・白目・瞳は固定にして、どの色でも顔が見分けられるようにしている。RN は `setPlayerPalette` 意図で色を送り、シーンはメッシュを作り直さずマテリアルの色だけを塗り直す。空の指定なら既定の色に戻る。色の決め方は住人と同じ `resolvePartColor`（`lib/rpg-hub/palette.ts`）で、プレイヤー専用の作りにはしていない。本人の色は `store/appearanceStore.ts` が持つ。DBからの読み込みは #253 で足す予定で、それまでは既定の色のまま表示される。
