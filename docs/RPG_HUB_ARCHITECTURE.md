...
`buildObject` は1オブジェクトにつき `TransformNode` を1つ作り、パーツの数だけメッシュを生成する。**同じ形状でもメッシュを共有していない**ため、メッシュ数はそのままドローコール数になる。下に書いた Thin Instances は未実装で、装飾物を増やし続けるならここが最初に効いてくる。実機で測ってから判断する。
