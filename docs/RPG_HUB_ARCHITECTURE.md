...
初期マップは **75オブジェクト / 361メッシュ**（地面とプレイヤーを含む）。内訳は建物4棟、道のタイル19枚、木12本、低木10本、草むら12個、岩6個、街灯6本、花壇4つ、NPC2体、プレイヤー1体。

\#214 で 181メッシュから倍増している。プレイヤーを23メッシュのカエルにし、木・低木・岩の造形を作り込み（1個あたり1〜4メッシュ増）、草むらを足したため。影を描くパスでは、このうち道のタイルと草むらを除いた**約290メッシュを深度だけでもう一度描く**ので、1フレームあたりのドローコールは合計で650前後になる。

**ここが次に効いてくる。** 装飾物は非インタラクティブで動かないため、下に書いた Thin Instances がそのまま使える（同じ `model` のオブジェクトをまとめると、装飾のドローコールは265から30程度まで落ちる）。実機で重ければ、影や解像度を削る前にこちらを先に入れる。
