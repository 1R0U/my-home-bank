...
- [x] WebView 内に灰色の地面と赤いカプセルが表示される（真っ黒・真っ白でない）
- [x] 画面左上に「残高: -」が表示される
- [x] カプセルをタップすると、RN 側のコンソールに `[babylon-spike] WebView → RN: { event: 'tapped', id: 'player' }` が出る／画面下部の「最新イベント」表示が更新される
- [x] 「残高を送る」ボタンを押すと、WebView 内の「残高: -」が「残高: 1350」（押すたび +100）に変わる
- [x] 別画面へ遷移 → 戻る、を数回繰り返してシーンが毎回再表示される
- [x] アプリをバックグラウンドに送り、復帰させてもシーンが継続する

> 注: 今回は「両端末で一通り動作する」ことの確認まで。端末個別の OS / Expo Go の詳細バージョン文字列と、各項目の細かな計測（FPS・メモリ・入力遅延）は未記録。これらは後続（下記「未確認・後続」）で扱う。
